...
# 添加命令行参数
parser.add_argument('--startpage', type=int, help='the start page to scrawl')
...
parser.add_argument('--pdfdir', type=str, help='The dir where you store pdfs')
# 解析命令行参数
...
from oldCode.timevar import savetime
import time 
import random
...
from oldCode.geteachinfo import readdata1
import glob
# pdf_dir = '/Volumes/Lexar1TB/PaperCrawl/PubMed/downloaded_pdfs/anaesthesiology/*.pdf'
file_list = glob.glob(pdfdir)
number_list = []
...
def spider_pubmed_paper_metadata(parameter: str = "computer science", startpage: int = 1,  endpage: int = 12):
    # print(f'\033[1;35m *******calling spider_pubmed_paper_metadata*****\033[0m')
    ...
    for i in range(startpage, endpage):
        ...
        # print(f'\033[1;35m **********1**********url is {url}\033[0m')
        request=urllib.request.Request(url,headers=header)
        response=urllib.request.urlopen(request)
        # print(f'\033[1;35m *********2**********url is {url}\033[0m')
        html=response.read()
...
def download_pubmed_pdf_using_paper_metadata(metadata):
    ...
    # print(f"enter download pubmed pdf using metadata with metadata {metadata}")
    ...
    for file in file_list:
        ...
        if PMID in number_list:
            print(f"\033[1;33m PMID {PMID} exists!\033[0m")
            return None
    ...
    PMCID：
    '''
    # print(f"\033[1;35m PMCID is {PMCID}\033[0m")
...
# metadatas = fetch_pdf_urls_from_pubmed(keyword = args.keywords, startpage = 1, endpage = args.endpage)
# print(f"\033[1;35m metadatas = {metadatas}\033[0m")
import logging
...
#         savepath = pdfdir[:-5] +  metadata[4] + ".pdf"
...
def crawlpage(page_idx: int = 1, parameter: str = "computer science"):
    ...
    # print(f"crawling page {page_idx}")
    baseurl = "https://pubmed.ncbi.nlm.nih.gov/"
    url=""
    header={"user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.4951.41 Safari/537.36 Edg/101.0.1210.32"}
    result_all = -1
    ...
    try:
        ...
        print(e)
        return None
    # print(f"returning len {len(data)}")
    return data
...
def download(item):
    try:
        ...
        if pdfstream is None:
            return 
        # savepath = f"./document/neurosurgery/{metadata[4]}.pdf"  # 这里用 PMID 存文件
        savepath = args.pdfdir[:-5] +  item[4] + ".pdf"
        file = open(savepath, 'wb')
        file.write(pdfstream)
        file.close()
        print('success')
...
def crawlpage_with_progress(page_idx, progress_bar):
    items = crawlpage(page_idx, Search_param(args.keywords).gen_search_param())
    progress_bar.update(1)
    return items 

def fetch_and_download(page_idx, progress_bar):
    # items = crawlpage(parameter = args.keywords, page_idx=page_idx)
    items = crawlpage_with_progress(page_idx, progress_bar)
    with concurrent.futures.ThreadPoolExecutor() as executor:
        executor.map(download, items)

def main(startpage, endpage):
    total_pages = endpage - startpage
    with tqdm(total=total_pages, desc="crawling page") as progress_bar:
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [executor.submit(fetch_and_download, page, progress_bar) for page in range(startpage, endpage)]
...
if __name__ == "__main__":
    startpage = args.startpage  # 根据需要设置
